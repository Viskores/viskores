...
# -----------------------------------------------------------------------------
function(viskores_test_install)
  if(NOT Viskores_INSTALL_ONLY_LIBRARIES)
    ...
    foreach(module IN LISTS all_modules)
      if(NOT TARGET ${module})
        viskores_module_get_property(directory ${module} DIRECTORY)
        file(RELATIVE_PATH directory "${Viskores_SOURCE_DIR}/viskores" "${directory}")
    ...
    set_tests_properties(TestInstallSetup PROPERTIES FIXTURES_SETUP viskores_installed)
    set_tests_properties(SourceInInstall PROPERTIES FIXTURES_REQUIRED viskores_installed)
    set_tests_properties(TestInstallCleanup PROPERTIES FIXTURES_CLEANUP viskores_installed)
...
# -----------------------------------------------------------------------------
function(viskores_generate_install_build_options file_loc_var)
#This generated file ensures that the adaptor's CMakeCache ends up with
#the same CMAKE_PREFIX_PATH that Viskores's does, even if that has multiple
...
# -----------------------------------------------------------------------------
function(viskores_test_against_install_cmake dir)
  set(name ${dir})
  set(install_prefix "${Viskores_BINARY_DIR}/CMakeFiles/_tmp_install")
  set(src_dir "${CMAKE_CURRENT_SOURCE_DIR}/${name}/")
  set(build_dir "${Viskores_BINARY_DIR}/CMakeFiles/_tmp_build/test_${name}_cmake/")
  ...
  if(CMAKE_VERSION VERSION_LESS 3.13)
    ...
    viskores_generate_install_build_options(build_config)
    set(args -C ${build_config})
  endif()

  if(WIN32 AND TARGET viskores_tbb)
...
# -----------------------------------------------------------------------------
function(viskores_test_against_install_make dir)
  ...
  set(viskores_install_dir "${Viskores_BINARY_DIR}/CMakeFiles/_tmp_install")
  if (WIN32)
    string(REPLACE ";" "\\;" escaped_path "$ENV{PATH}")
    list(APPEND env_vars "PATH=$<SHELL_PATH:${viskores_install_dir}/bin>\\;${escaped_path}")
  elseif(APPLE)
    list(APPEND env_vars "DYLD_LIBRARY_PATH=${viskores_install_dir}/lib:$ENV{DYLD_LIBRARY_PATH}")
  else()
    list(APPEND env_vars "LD_LIBRARY_PATH=${viskores_install_dir}/lib:$ENV{LD_LIBRARY_PATH}")
  endif()

  # The plain make test uses the given dir as its test name
  _test_install_make(${dir} ${dir}
    "VISKORES_INSTALL_PREFIX=${viskores_install_dir}"
    "VISKORES_CONFIG_MK_PATH=${viskores_install_dir}/share/viskores-${Viskores_VERSION_MAJOR}.${Viskores_VERSION_MINOR}/viskores_config.mk"
    ${env_vars}
  )
