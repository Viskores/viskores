...
## NumPy Output: Zero-Copy View vs. Copy

`viskores.cont.UnknownArrayHandle.asnumpy` and `viskores.cont.asnumpy` return a
zero-copy NumPy view when the source `ArrayHandle` was originally created from
a NumPy array via `array_from_numpy`. For any other source (for example, an
`ArrayHandle` allocated by Viskores as the output of a filter), the data is
copied into a fresh NumPy array. This keeps the returned NumPy array safe from
later reallocations of the Viskores buffer that the binding cannot detect or
prevent.
