                                    ...
                                    const std::function<void(const char*)>& erase_existing_name)
{
  ...
  auto make_test_data_set =
    nb::class_<viskores::cont::testing::MakeTestDataSet>(m, "MakeTestDataSet");

#define VISKORES_NB_MAKE_TEST_DATASET_METHOD(name)                                               \
  make_test_data_set.def(#name,                                                                   \
                         [](viskores::cont::testing::MakeTestDataSet& self)                      \
                         {                                                                       \
                           return WrapDataSet(self.name());                                       \
                         })
                                   ...
                                   const std::function<void(const char*)>& erase_existing_name)
{
  ...
  nb::class_<viskores::source::Tangle>(m, "Tangle")
    .def(nb::init<>())
    .def("__repr__",
  ...
  nb::class_<viskores::source::PerlinNoise>(m, "PerlinNoise")
    .def(nb::init<>())
    .def("__repr__",
         [](const viskores::source::PerlinNoise& self) {
           const auto dims = self.GetPointDimensions();
           std::ostringstream stream;
           stream << "viskores.source.PerlinNoise(point_dimensions=(" << dims[0] << ", "
                  << dims[1] << ", " << dims[2] << "))";
           return stream.str();
         })
    .def("GetPointDimensions",
  ...
  nb::class_<viskores::source::Oscillator>(m, "Oscillator")
    .def(nb::init<>())
    .def("__repr__",
         [](const viskores::source::Oscillator& self) {
           const auto dims = self.GetPointDimensions();
           std::ostringstream stream;
           stream << "viskores.source.Oscillator(point_dimensions=(" << dims[0] << ", "
                  << dims[1] << ", " << dims[2] << "))";
           return stream.str();
         })
    .def("GetPointDimensions",
  ...
  nb::class_<viskores::source::Wavelet>(m, "Wavelet")
    .def(nb::init<>())
    .def("__repr__",
...
void RegisterNanobindIOClasses(nb::module_& m, const std::function<void(const char*)>& erase_existing_name)
{
#if VISKORES_PYTHON_ENABLE_IO_HDF5
  erase_existing_name("ImageReaderHDF5");
  nb::class_<viskores::io::ImageReaderHDF5>(m, "ImageReaderHDF5")
  ...
  nb::class_<viskores::io::ImageWriterHDF5>(m, "ImageWriterHDF5")
    .def(nb::init<const std::string&>(), nb::arg("file_name"))
    .def("WriteDataSet",
  ...
  nb::class_<viskores::io::ImageReaderPNG>(m, "ImageReaderPNG")
    .def(nb::init<const std::string&>(), nb::arg("file_name"))
    .def("ReadDataSet",
  ...
  nb::class_<viskores::io::ImageReaderPNM>(m, "ImageReaderPNM")
    .def(nb::init<const std::string&>(), nb::arg("file_name"))
    .def("ReadDataSet",
  ...
  nb::class_<viskores::io::ImageWriterPNG>(m, "ImageWriterPNG")
    .def(nb::init<const std::string&>(), nb::arg("file_name"))
    .def("WriteDataSet",
  ...
  nb::class_<viskores::io::ImageWriterPNM>(m, "ImageWriterPNM")
    .def(nb::init<const std::string&>(), nb::arg("file_name"))
    .def("WriteDataSet",
  ...
  nb::class_<viskores::io::VTKDataSetWriter>(m, "VTKDataSetWriter")
    .def(nb::init<const std::string&>(), nb::arg("file_name"))
    .def("WriteDataSet",
