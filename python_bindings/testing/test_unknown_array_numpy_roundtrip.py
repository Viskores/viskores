...
def check_double_wrap_keeps_view_path():
    ...
    # zero-copy view path. Tracking shareability with a plain set would treat
    # both registrations as one entry and falsely demote the second wrapper to
    # the copy path as soon as the first wrapper is destroyed.
...
def check_copy_kwarg_accepts_unshareable(dtype):
    ...
    base[0] = mutation_value(dtype, 99)
    np.testing.assert_array_equal(unknown.asnumpy(), expected)

    readonly = scalar_values(dtype)
    readonly.flags.writeable = False
