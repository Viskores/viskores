...
def main():
    ...
    script = repo_root / "python_bindings/examples/clipping/Clipping.py"

    with tempfile.TemporaryDirectory() as temp_dir:
        temp_path = Path(temp_dir)
        input_path = temp_path / "input.vtk"
        output_path = temp_path / "output.vtk"
        ...
        subprocess.run(
            [sys.executable, str(script), str(input_path), "pointvar", "0.0", str(output_path)],
            check=True,
            cwd=temp_path,
            env=env,
        )
