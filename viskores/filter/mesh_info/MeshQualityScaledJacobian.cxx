...
#include <viskores/filter/mesh_info/worklet/cellmetrics/CellScaledJacobianMetric.h>

namespace
{

struct ScaledJacobianWorklet : MeshQualityWorklet<ScaledJacobianWorklet>
{
...
MeshQualityScaledJacobian::MeshQualityScaledJacobian()
{
  ...
  this->SetOutputFieldName("scaledJacobian");
}

viskores::cont::DataSet MeshQualityScaledJacobian::DoExecute(const viskores::cont::DataSet& input)
{
  viskores::cont::UnknownArrayHandle outArray =
    ScaledJacobianWorklet{}.Run(input, this->GetFieldFromDataSet(input));
