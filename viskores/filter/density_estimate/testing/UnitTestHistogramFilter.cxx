...
//
void VerifyHistogram(const viskores::cont::DataSet& result,
                     viskores::Id numberOfBins,
                     const viskores::Range& range,
                     viskores::Float64 delta,
                     bool output = true)
{
  VISKORES_TEST_ASSERT(result.HasField("histogram"), "Output field missing");

  viskores::cont::ArrayHandle<viskores::Id> bins;
  result.GetField("histogram").GetData().AsArrayHandle(bins);

  viskores::cont::ArrayHandle<viskores::Id>::ReadPortalType binPortal = bins.ReadPortal();

  viskores::Id sum = 0;
  for (viskores::Id i = 0; i < numberOfBins; i++)
  {
    viskores::Float64 lo = range.Min + (static_cast<viskores::Float64>(i) * delta);
    viskores::Float64 hi = lo + delta;
    sum += binPortal.Get(i);
    if (output)
    {
  ...
  VISKORES_TEST_ASSERT(test_equal(sum, 1000), "Histogram not full");
}
...
{
  return viskores::cont::testing::Testing::Run(TestHistogram, argc, argv);
}
