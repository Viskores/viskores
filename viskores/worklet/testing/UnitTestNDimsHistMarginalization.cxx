...
#include <viskores/worklet/NDimsHistMarginalization.h>
...
{
  VISKORES_EXEC
  bool operator()(viskores::Id var, viskores::Id binId) const
  {
    if (var == 1)
    {
      if (1 < binId && binId < 4)
...
void TestNDimsHistMarginalization()
{
  ...
  viskores::Range rangeFieldA;
  viskores::Float64 deltaFieldA;
  ndHistogram.AddField(ds.GetField("fieldA").GetData(), 10, rangeFieldA, deltaFieldA);

  viskores::Range rangeFieldB;
  viskores::Float64 deltaFieldB;
  ndHistogram.AddField(ds.GetField("fieldB").GetData(), 10, rangeFieldB, deltaFieldB);

  viskores::Range rangeFieldC;
  viskores::Float64 deltaFieldC;
  ...
  viskores::cont::ArrayHandle<bool> marginalVariable =
    viskores::cont::make_ArrayHandle({ true, false, true });

  viskores::cont::ArrayHandle<viskores::Id> numberOfBins =
    viskores::cont::make_ArrayHandleMove(std::vector<viskores::Id>(3, 10));

  // calculate marginal histogram by the setup (return sparse represetnation)
  std::vector<viskores::cont::ArrayHandle<viskores::Id>> marginalBinIds;
  viskores::cont::ArrayHandle<viskores::Id> marginalFreqs;
  viskores::worklet::NDimsHistMarginalization ndHistMarginalization;
  ...
  {
    viskores::Id idx0 = marginalBinIds[0].WritePortal().Get(i);
...
{
  return viskores::cont::testing::Testing::Run(TestNDimsHistMarginalization, argc, argv);
}
