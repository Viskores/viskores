...
#include <viskores/worklet/FieldStatistics.h>
...
//
viskores::cont::DataSet Make2DUniformStatDataSet0()
{
  ...
  // Create cell scalar
  viskores::Float32 data[nVerts] = { 4, 1, 10, 6, 8, 2, 9, 3, 5, 7 };
  dataSet.AddField(viskores::cont::make_Field(
    "data", viskores::cont::Field::Association::Cells, data, nCells, viskores::CopyFlag::On));

  viskores::cont::CellSetStructured<dimension> cellSet;

  //Set uniform structure
  cellSet.SetPointDimensions(viskores::make_Vec(xVerts, yVerts));
...
//
viskores::cont::DataSet Make2DUniformStatDataSet1()
{
  viskores::cont::DataSet dataSet;
  ...
  viskores::Float32 poisson[nVerts] = {
  ...
  viskores::Float32 normal[nVerts] = {
  ...
  viskores::Float32 chiSquare[nVerts] = {
  ...
  viskores::Float32 uniform[nVerts] = {
...
void PrintStatInfo(viskores::worklet::FieldStatistics<viskores::Float32>::StatInfo statinfo)
{
...
void TestFieldSimple()
{
  ...
  viskores::worklet::FieldStatistics<viskores::Float32>::StatInfo statinfo;

  // Data attached is [1:10]
  viskores::cont::DataSet ds = Make2DUniformStatDataSet0();
  ...
  // Run
  viskores::worklet::FieldStatistics<viskores::Float32>().Run(data, statinfo);
  std::cout << "Statistics for CELL data:" << std::endl;
  PrintStatInfo(statinfo);

  viskores::Float32 expected[8] = { 6.0f, 1.0f, 10.0f, 5.5f, 8.25f, 2.87228f, 0.0f, 1.77576f };
  VISKORES_TEST_ASSERT(test_equal(statinfo.median, expected[0]), "Error in median");
  VISKORES_TEST_ASSERT(test_equal(statinfo.minimum, expected[1]), "Error in minimum");
  VISKORES_TEST_ASSERT(test_equal(statinfo.maximum, expected[2]), "Error in maximum");
  VISKORES_TEST_ASSERT(test_equal(statinfo.mean, expected[3]), "Error in mean");
  VISKORES_TEST_ASSERT(test_equal(statinfo.variance, expected[4]), "Error in variance");
  VISKORES_TEST_ASSERT(test_equal(statinfo.stddev, expected[5]), "Error in stddev");
  VISKORES_TEST_ASSERT(test_equal(statinfo.skewness, expected[6]), "Error in skewness");
  VISKORES_TEST_ASSERT(test_equal(statinfo.kurtosis, expected[7]), "Error in kurtosis");
  VISKORES_DEPRECATED_SUPPRESS_END
}
...
void TestFieldStandardDistributions()
{
  ...
  viskores::cont::DataSet ds = Make2DUniformStatDataSet1();

  // Point data
  viskores::cont::ArrayHandle<viskores::Float32> p_poisson;
  ds.GetField("p_poisson").GetData().AsArrayHandle(p_poisson);
  viskores::cont::ArrayHandle<viskores::Float32> p_normal;
  ds.GetField("p_normal").GetData().AsArrayHandle(p_normal);
  viskores::cont::ArrayHandle<viskores::Float32> p_chiSquare;
  ds.GetField("p_chiSquare").GetData().AsArrayHandle(p_chiSquare);
  viskores::cont::ArrayHandle<viskores::Float32> p_uniform;
  ds.GetField("p_uniform").GetData().AsArrayHandle(p_uniform);

  // Run Poisson data
  viskores::worklet::FieldStatistics<viskores::Float32>().Run(p_poisson, statinfo);
  std::cout << "Poisson distributed POINT data:" << std::endl;
  PrintStatInfo(statinfo);

  // Run Normal data
  viskores::worklet::FieldStatistics<viskores::Float32>().Run(p_normal, statinfo);
  std::cout << "Normal distributed POINT data:" << std::endl;
  PrintStatInfo(statinfo);

  // Run Chi Square data
  viskores::worklet::FieldStatistics<viskores::Float32>().Run(p_chiSquare, statinfo);
  std::cout << "Chi Square distributed POINT data:" << std::endl;
  PrintStatInfo(statinfo);

  // Run Uniform data
  viskores::worklet::FieldStatistics<viskores::Float32>().Run(p_uniform, statinfo);
...
{
  return viskores::cont::testing::Testing::Run(TestFieldStatistics, argc, argv);
}
