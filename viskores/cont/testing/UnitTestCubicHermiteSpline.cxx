                     ...
                     const std::vector<viskores::Vec3f>& posAnswer)
{
  ...
  VISKORES_TEST_ASSERT(
    test_equal_ArrayHandles(posResult, viskores::cont::make_ArrayHandle(posAnswer, viskores::CopyFlag::Off)));
}
...
void CheckEvaluation(const viskores::cont::CubicHermiteSpline& spline,
                     ...
                     const std::vector<viskores::Vec3f>& posAnswer)
{
  return CheckEvaluation(spline, viskores::cont::make_ArrayHandle(params, viskores::CopyFlag::Off), posAnswer);
}
...
void SaveSamples(viskores::cont::CubicHermiteSpline& spline)
{
  ...
  for (viskores::Id i = 0; i < pts.GetNumberOfValues(); ++i)
    fout <<i<<", "<<pts.Get(i)[0]<<", "<<pts.Get(i)[1]<<", "<<pts.Get(i)[2]<<std::endl;
  fout.close();
  ...
  invoke(SplineEvalWorklet{}, viskores::cont::make_ArrayHandle(params, viskores::CopyFlag::Off), spline, posResults);

  auto pos = posResults.ReadPortal();
  for (viskores::Id i = 0; i < pos.GetNumberOfValues(); i++)
    fout<<params[i]<<", "<<pos.Get(i)[0]<<", "<<pos.Get(i)[1]<<", "<<pos.Get(i)[2]<<std::endl;
  fout.close();
}
