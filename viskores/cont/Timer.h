...
/// A class that can be used to time operations in Viskores that might be occurring
...
/// elapsed time of all enabled devices will be returned. Normally cuda is expected to
...
/// the same timer to measure the cuda kernel call as well as the cuda device execution.
...
public:
  ...
  ///
  /// Note that this method will always block until the device(s) finish even if the
  ...
  /// records the time as fences are encountered. But regardless, this `Synchronize` method
